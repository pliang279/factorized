...
class EFLSTM(nn.Module):
	...
	def forward(self, x):
		# x is t x n x d
		n = x.shape[1]
		self.hx = torch.zeros(n, h).cuda()
		self.cx = torch.zeros(n, h).cuda()
		all_hs = []
		all_cs = []
		for i in range(t):
...
def train_mfn(X_train, y_train, X_valid, y_valid, X_test, y_test, configs):
	...
	for epoch in range(config["num_epochs"]):
		...
		if valid_loss <= best_valid:
			...
			torch.save(model, 'res/mfn_%d.pt' %rand)
		else:
			print epoch, train_loss, valid_loss, test_loss

	model = torch.load('res/mfn_%d.pt' %rand)
...
while True:
	...
	hl = 64 #random.choice([32,64,88,128,156,256])
	ha = 8 #random.choice([8,16,32,48,64,80])
	hv = 80 #random.choice([8,16,32,48,64,80])
	config["h_dims"] = [hl,ha,hv]
	config["memsize"] = 400 #random.choice([64,128,256,300,400])
	config["windowsize"] = 2
	config["batchsize"] = 128 #random.choice([32,64,128,256])
	config["num_epochs"] = 200
	config["lr"] = 0.01 #random.choice([0.001,0.002,0.005,0.008,0.01])
	...
	NN2Config["drop"] = 0.2 #random.choice([0.0,0.2,0.5,0.7])
	gamma1Config = dict()
	...
	outConfig = dict()
	outConfig["shapes"] = 256 #random.choice([32,64,128,256])
