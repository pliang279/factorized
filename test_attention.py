...
class SelfAttention(nn.Module):
	def __init__(self, d, h, t, output_dim, dropout): #, n_layers, bidirectional, dropout):
		...
		self.dropout = nn.Dropout(dropout)
		#print 't is', t
		self.alpha = nn.Parameter(torch.Tensor(t,1).cuda())		# change init if bad results
		#print 'init alpha', self.alpha.shape
	...
	def forward(self, x):
		...
		temp = torch.bmm(x,x.permute(0,2,1)) 	# n x t x t
		#print temp.shape
		alphas = self.alpha.repeat(1,t)			# t x t
		#print 'tiled alpha', alphas.shape
		attention = alphas * temp				# n x t x t
		#print attention.shape
		x = torch.bmm(attention,x)				# n x t x d
		#print x.shape
		x = x.permute(1,0,2)					# t x n x d
...
def train_self_attention(X_train, y_train, X_valid, y_valid, X_test, y_test, config):
	...
	model = SelfAttention(d,h,t,output_dim,dropout)
...
train_self_attention(X_train, y_train, X_valid, y_valid, X_test, y_test, config)
assert False
